...
from scripts.scanner import WINDOW_MS, extract
...
def test_extract_default_now_ms_uses_time(isolated_claude_home, monkeypatch):
    ...
    import scripts.scanner as extractor_mod
...
def test_extract_populates_v0_11_per_name_maps(isolated_claude_home):
    """The per-name maps are emitted and each sums to its scalar total."""
    _write_jsonl(
        isolated_claude_home / "projects" / "-foo" / "s1.jsonl",
        [
            {"timestamp": "2026-01-01T00:00:00Z"},
            ...
            },
            {
                ...
                "message": {
                    "role": "user",
                    "content": "<command-name>superpowers:tdd</command-name>",
                },
            },
            {
                "type": "assistant",
    ...
    s = out.sessions[0]
    # Deterministic per-name maps for the keyboard-side surfaces.
    assert s.skill_invocations_by_name == {"plan": 1}
    assert s.plugin_invocations_by_name == {"superpowers:tdd": 1}
    # Invariant: every per-name map sums to its scalar counter.
    assert sum(s.skill_invocations_by_name.values()) == s.user_skill_invocations
    assert sum(s.mcp_invocations_by_name.values()) == s.hitl_mcp_invocations
    assert sum(s.plugin_invocations_by_name.values()) == s.plugin_invocations
...
def test_extract_populates_v0_4_turn_partition_fields(isolated_claude_home):
    """End-to-end: a JSONL with a user msg + a sidechain subagent + a final
    main-thread end_turn produces a single AFK turn and a 10-minute
    parallel-subagent contribution.

    Under the v0.4 turn rule a turn opens at the USER event and closes at
    either ``stop_reason == end_turn`` or session end. The single 10-min
    span ends up classified as one AFK turn (duration > 5 min).
    """
    base_min = 27_810_000
    base_ms = base_min * 60_000
    end_ms = base_ms + 10 * 60_000  # 10 minutes later
    ...
    # 11 sidechain subagent events, one per minute, sharing subagent_id.
    for i in range(11):
        lines.append(
            {
                "type": "assistant",
    ...
    # Main-thread agent activity every minute so each intra-turn gap
    # stays ≤ K_TURN_SECONDS (5 min). Without these, the v0.10 active-
    # seconds rule would clip the single 10-min gap at 5 min and
    # mislabel the turn as HITL.
    for i in range(1, 10):
        lines.append(
            {
                "type": "assistant",
    ...
    # Main-thread end_turn — closes the AFK turn at the 10-min mark.
    lines.append(
        {
            "type": "assistant",
    ...
    out = extract(
        ...
        now_ms=end_ms + 10 * 60_000,
    )
    assert len(out.sessions) == 1
    s = out.sessions[0]
    # One AFK turn spanning 10 min — turn duration > 300s threshold.
    assert s.hitl_minutes == 0
    assert s.afk_minutes == 10
    # subagent active [base_ms, base_ms + 10 min] ∩ AFK turn = 10 min.
    assert s.afk_parallel_minutes_foreground == 10
    assert s.afk_max_streak_minutes == 10
    assert s.cron_parallel_minutes == 0
    # One AFK interval covering the AFK turn (minute-space).
    assert len(s.afk_intervals) == 1
    ivl = s.afk_intervals[0]
    assert ivl.is_cron is False
    # Turn spans 10 min in seconds → 11 minute buckets when expressed as
    # ``[start_minute, floor(end_ts/60_000) + 1)``.
    assert ivl.end_minute_exclusive - ivl.start_minute == 11
...
def test_extract_v0_5_redundant_approvals_populated(isolated_claude_home):
    ...
    # Two DENIED git calls -> friction count 2.
    lines.append(_tool_use(0, "git push --force", "tu_0"))
    lines.append(
        _tool_result(0, "tu_0", "The user's tool use was rejected.", True)
    )
    lines.append(_tool_use(1, "git reset --hard", "tu_1"))
    lines.append(
        _tool_result(
            1, "tu_1", "Permission for this action was denied.", True
        )
    )
    # One GRANTED ls call (non-denial result) -> no friction.
    lines.append(_tool_use(2, "ls -la", "tu_2"))
    lines.append(_tool_result(2, "tu_2", "file1\nfile2\n", False))
    ...
    s = out.sessions[0]
    assert s.redundant_approvals_per_signature == {"Bash::git": 2}


def test_extract_v0_5_redundant_approvals_edit_signature_is_hashed(
    isolated_claude_home,
):
    """A DENIED Write maps to the hashed top-level dir signature so the
    directory name never crosses the wire."""
    import hashlib
                ...
                "content": [
                    {
                        ...
                        "id": "tu_w",
                        "input": {
                            "file_path": "/secrettopdir/file_0.py",
                            "content": "x=1\n",
                        },
                    }
                ],
                ...
                "content": [
                    {
                        ...
                        "is_error": True,
                        "content": (
                            "The user doesn't want to proceed with this "
                            "tool use."
                        ),
                    }
                ],
...
def test_extract_v0_6_hitl_mcp_invocations_populated(isolated_claude_home):
    """MCP tool calls inside HITL turns are counted; AFK ones aren't.

    Turn 1: USER → MCP → end_turn within 60s → HITL turn → MCP counts.
    Turn 2: USER → MCP → end_turn 10 min later → AFK turn → MCP doesn't.
    """
    base_min = 27_810_000
    base_ms = base_min * 60_000
    _write_jsonl(
        ...
        [
            ...
            {
                "type": "assistant",
                "timestamp": _iso(base_ms + 31 * 60_000),
                "message": {
                    "role": "assistant",
                    "model": "claude-sonnet-4-6",
                    "content": [
