...
| `afk_dispatch_minutes`             | integer           | no       | Of `afk_minutes`, the portion inside `Agent`/`Task` subagent-dispatch intervals. SUBTRACTED from the leverage numerator (the subagent work is counted via `afk_parallel_minutes_foreground`); does NOT affect `afk_max_streak_minutes` / longest run. |
