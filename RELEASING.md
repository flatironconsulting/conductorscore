...
1. Bump the version in **both** the root `VERSION` file and `pyproject.toml`
   (they must stay in lockstep — CI and the skill package both read them).
...
10. Server: update vendored `WIRE_FORMAT.md` if it changed; extend the Zod
    accept-list to the new version.
